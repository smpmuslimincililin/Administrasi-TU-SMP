            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-3">
                <button
                  ...
                  className={`flex flex-col items-center justify-center gap-1 sm:gap-1.5 py-3 sm:py-4 px-2 sm:px-3 rounded-xl border-2 font-medium transition-all duration-200 min-h-[44px] ${
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
        <div className="bg-sky-50 dark:bg-sky-900/20 p-3 sm:p-4 rounded-xl shadow-sm dark:shadow-gray-900/50 border border-sky-100 dark:border-sky-800 flex items-center gap-2 sm:gap-3">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-sky-200 dark:bg-sky-800 flex items-center justify-center shrink-0">
            <Users className="w-4 h-4 sm:w-5 sm:h-5 text-sky-700 dark:text-sky-300" />
          </div>
          <div className="min-w-0">
            <p className="text-sky-700/80 dark:text-sky-400/80 text-xs sm:text-sm truncate">
              Total Siswa
            </p>
            <p className="text-lg sm:text-xl md:text-2xl font-bold text-sky-900 dark:text-sky-200">
              {stats.totalSiswa}
            </p>
          </div>
        </div>
        <div className="bg-teal-50 dark:bg-teal-900/20 p-3 sm:p-4 rounded-xl shadow-sm dark:shadow-gray-900/50 border border-teal-100 dark:border-teal-800 flex items-center gap-2 sm:gap-3">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-teal-200 dark:bg-teal-800 flex items-center justify-center shrink-0">
            <TrendingUp className="w-4 h-4 sm:w-5 sm:h-5 text-teal-700 dark:text-teal-300" />
          </div>
          <div className="min-w-0">
            <p className="text-teal-700/80 dark:text-teal-400/80 text-xs sm:text-sm truncate">
              Progress Positif
            </p>
            <p className="text-lg sm:text-xl md:text-2xl font-bold text-teal-900 dark:text-teal-200">
              {stats.progressPositif}
            </p>
          </div>
        </div>
        <div className="bg-pink-50 dark:bg-pink-900/20 p-3 sm:p-4 rounded-xl shadow-sm dark:shadow-gray-900/50 border border-pink-100 dark:border-pink-800 flex items-center gap-2 sm:gap-3">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-pink-200 dark:bg-pink-800 flex items-center justify-center shrink-0">
            <AlertCircle className="w-4 h-4 sm:w-5 sm:h-5 text-pink-700 dark:text-pink-300" />
          </div>
          <div className="min-w-0">
            <p className="text-pink-700/80 dark:text-pink-400/80 text-xs sm:text-sm truncate">
              Perlu Perhatian
            </p>
            <p className="text-lg sm:text-xl md:text-2xl font-bold text-pink-900 dark:text-pink-200">
              {stats.perluPerhatian}
            </p>
          </div>
        </div>
        <div className="bg-purple-50 dark:bg-purple-900/20 p-3 sm:p-4 rounded-xl shadow-sm dark:shadow-gray-900/50 border border-purple-100 dark:border-purple-800 flex items-center gap-2 sm:gap-3">
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-purple-200 dark:bg-purple-800 flex items-center justify-center shrink-0">
            <Info className="w-4 h-4 sm:w-5 sm:h-5 text-purple-700 dark:text-purple-300" />
          </div>
          <div className="min-w-0">
            <p className="text-purple-700/80 dark:text-purple-400/80 text-xs sm:text-sm truncate">
              Catatan Biasa
            </p>
            <p className="text-lg sm:text-xl md:text-2xl font-bold text-purple-900 dark:text-purple-200">
              {stats.catatanBiasa}
            </p>
          </div>
        </div>
...
const CatatanSiswa = ({ user, onShowToast }) => {
  ...
  // FIX: default value ini cuma dipakai sesaat sebelum activeYear dari DB kebaca,
  // dan sekarang loadDashboardData sudah nunggu academicYear yang benar duluan.
  // Tetap diupdate ke tahun ajaran terbaru biar aman kalau suatu saat urutan
  // pemanggilan berubah lagi.
  ...
  const getCurrentUser = async () => {
    try {
      ...
      // FIX: Ambil tahun ajaran aktif DULU, sebelum setCurrentUser dipanggil.
      // Ini penting karena setCurrentUser men-trigger useEffect [currentUser, currentClass]
      // yang langsung memanggil loadDashboardData(). Kalau academicYear/semester belum
      ...
      // setCurrentUser dipanggil PALING TERAKHIR, setelah academicYear & currentClass
      // sudah pasti benar. Karena React membatch semua setState di dalam function ini,
      // begitu useEffect [currentUser, currentClass] jalan, academicYear & semester
      // sudah pasti nilai yang benar juga.
  ...
  const loadStudentNotes = async (studentId) => {
    ...
    try {
      ...
      // FIX: Sebelumnya nama guru diambil lewat query terpisah untuk TIAP catatan
      // (N+1 query problem) — kalau siswa punya 20 catatan, itu 20 query tambahan
      // sekaligus. Sekarang nama guru diambil langsung dalam 1 query yang sama
      // pakai embedded select via foreign key teacher_id -> users, jadi cuma
      // 1 round-trip ke database berapa pun banyaknya catatan.
      ...
      const notesWithTeacher = (data || []).map((note) => ({
        ...note,
        teacher_name: note.teacher?.full_name || "Guru",
      }));
  ...
  // FIX: class dark mode ditulis statis langsung di sini (bukan di-generate
  // pakai .replace() saat runtime kayak sebelumnya). Tailwind cuma bisa
  // mendeteksi & compile class yang benar-benar tertulis literal di source
  // code — class yang dirakit secara dinamis (mis. via string concat/replace)
  // tidak akan pernah masuk ke CSS hasil build, jadi warnanya nggak nyala
  // sama sekali di dark mode. Sekaligus benerin bug "dark:dark:bg-..." yang
  // muncul dari template string sebelumnya.
