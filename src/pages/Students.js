...
export const Students = ({ user: userFromProps, onShowToast, darkMode }) => {
  ...
  // ✅ NEW: Data siswa baru ditampilkan setelah Jenjang & Kelas dipilih.
  // Gender cuma filter tambahan opsional (boleh dipilih kapan saja, gak nge-gate tampilan).
  const filteredData = useMemo(() => {
    if (!selectedJenjang || !selectedKelas) return [];
    ...
    return siswaData.filter((siswa) => {
      ...
      const matchesJenjang = siswa.class_id?.startsWith(selectedJenjang);
      const matchesKelas = siswa.class_id === selectedKelas;
  ...
  return (
    <div className="min-h-screen p-4 sm:p-6 md:p-8 bg-gradient-to-b from-blue-50 to-white dark:from-gray-900 dark:to-gray-800 transition-colors duration-300">
      ...
      {/* ✅ NEW: Stats card — kasus 4 card (Wali Kelas) tetap grid 2x2 (2 kolom).
          Kasus lain (Admin/Guru BK, 2–5 card) tetap 1 baris penuh. */}
      <div
      ...
      {!selectedJenjang || !selectedKelas ? (
        <div className="mb-4 p-3 bg-amber-50 dark:bg-amber-900/20 rounded-lg border border-amber-200 dark:border-amber-800 text-sm inline-flex items-center gap-2">
          ...
          <span className="text-amber-800 dark:text-amber-300 font-medium">
            Pilih Jenjang dan Kelas terlebih dahulu untuk menampilkan data siswa.
          </span>
        </div>
      ) : (
        ...
        {filteredData.length > 0 ? (
          ...
          <div className="p-8 text-center text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              {!selectedJenjang || !selectedKelas
                ? "Pilih Jenjang dan Kelas dulu di atas untuk melihat data siswa."
